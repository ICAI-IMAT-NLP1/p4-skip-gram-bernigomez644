...
class SkipGramNeg(nn.Module):
    ...
    def forward_noise(self, batch_size: int, n_samples: int) -> torch.Tensor:
        ...
        if self.noise_dist is None:
            ...
            noise_dist: torch.Tensor = torch.ones(self.n_vocab) / self.n_vocab
        else:
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(
        ...
        dot = torch.bmm(input_vectors, output_vectors)
        sigmoid = torch.sigmoid(dot)
        out_loss = torch.log(sigmoid)
        # Compute log-sigmoid loss for incorrect classifications
        # TODO
        noise_vectors = noise_vectors.permute(0, 2, 1)
        negatice_dot = torch.bmm(input_vectors, noise_vectors)
        sigmoid = torch.sigmoid(-negatice_dot)
        noise_loss = torch.log(sigmoid)
        noise_loss_sum = noise_loss.sum(dim=2)

        average_loss = -(noise_loss_sum + out_loss).mean()
